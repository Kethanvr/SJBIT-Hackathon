  ...
  (
    ...
    const variantStyles = {
      default: `bg-white border ${
        isPro ? "border-yellow-100" : "border-blue-100"
      }`,
      outlined: `bg-transparent border ${
        isPro ? "border-yellow-200" : "border-blue-200"
      }`,
      elevated: `bg-white shadow-md ${
        isPro ? "shadow-yellow-100/50" : "shadow-blue-100/50"
      }`,
    ...
    const interactiveStyles = cn({
      ...
      [`hover:border-${isPro ? "yellow" : "blue"}-300`]:
        hoverable && (variant === "default" || variant === "outlined"),
      [`hover:shadow-${isPro ? "yellow" : "blue"}-200/50`]:
...
export const CardImage = forwardRef(
  (
    ...
    const imgStyles = cn("w-full h-full", {
    ...
    return position === "top" ? (
      <div className={containerStyles}>
        <img ref={ref} src={src} alt={alt} className={imgStyles} {...props} />
      </div>
    ) : (
